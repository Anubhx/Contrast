    ...
    <div className="w-full">
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          ...
          {/* Globe icon */}
          <svg className="ml-4 mr-1 w-[15px] h-[15px] text-text-tertiary shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" role="img" aria-label="Website URL icon">
          ...
          <input
            ...
            autoComplete="url"
            spellCheck="false"
            className="flex-1 border-none outline-none font-sans text-[14px] text-text-primary bg-transparent px-3 min-w-0 placeholder:text-text-placeholder"
            disabled={loading}
          />
          <button
            type="submit"
            disabled={loading || !url}
            className="bg-accent text-white border-none font-sans text-[13px] font-semibold px-[22px] h-[40px] m-1.5 rounded-full cursor-pointer tracking-[0.01em] whitespace-nowrap transition-all duration-150 hover:opacity-90 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-accent focus-visible:outline-offset-2 shrink-0 inline-flex items-center justify-center gap-[8px] min-w-[100px]"
          >
            {loading ? (
              <>
