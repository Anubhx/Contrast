...
export function LoadingView({ step }: LoadingViewProps) {
  return (
    <div className="relative z-10 flex flex-1 w-full max-w-[1600px] mx-auto min-h-[640px]">
      {/* Left Column for loading */}
      <div className="flex-[0.5] py-[64px] pr-[56px] pl-[40px] border-r border-border flex flex-col justify-center">
        <p className="text-[11px] font-mono text-text-tertiary tracking-[0.06em] mb-[6px] uppercase">
          Auditing
        </p>
        <h2 className="text-[22px] font-semibold text-text-primary tracking-[-0.02em] mb-[48px] truncate">
          Processing URL...
        </h2>
        <LoadingSequence currentStep={step} />
      </div>
      {/* Right Column for loading (mock preview) */}
      <div className="flex-[0.5] py-[40px] flex items-center justify-center bg-bg-subtle relative overflow-hidden">
